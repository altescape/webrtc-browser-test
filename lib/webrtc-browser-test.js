(function() {
  ...
  const setMic = function(microphone) {
    mic = microphone;
  };

  const micphone = function() {
    return mic;
  };
  ...
  const WebRtcBrowserTest = function(opts) {
    ...
    let videoElement = opts.mediaElementContainer.querySelector('video');
    if (!videoElement) {
    ...
    let audioElement = opts.mediaElementContainer.querySelector('audio');
    if (!audioElement) {
    ...
    const stopAll = function() {
      if (stream()) {
        if (mic && mic.context) { mic.context.close(); }
